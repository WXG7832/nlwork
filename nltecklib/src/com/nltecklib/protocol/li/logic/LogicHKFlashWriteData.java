...
/**
 * @author wavy_zheng
 * @version 创建时间：2020年4月21日 下午1:30:03 类说明
 */
public class LogicHKFlashWriteData extends Data implements Configable, Queryable, Responsable {
	...
	public void encode() {
		...
		data.add(isReverseDriverChnIndex() ? (byte) ProtocolUtil.reverseChnIndexInLogic(chnIndex,Data.getDriverChnCount())
				: (byte) chnIndex);
		// cc30个校准点
	...
	// private List<HKCalDot> decodeCalDot(int index) {
	//
	// List<HKCalDot> list = new ArrayList<HKCalDot>();
	// for (int i = 0; i < MODE_DOT_COUNT; i++) {
	//
	// HKCalDot dot = new HKCalDot();
	// dot.adc = (double)ProtocolUtil.compose(data.subList(index,
	// index+3).toArray(new Byte[0]), true) / 100;
	// index += 3;
	// dot.adcK = (double)ProtocolUtil.composeSpecialMinus(data.subList(index,
	// index+4).toArray(new Byte[0]), true) / Math.pow(10, KB_FACTOR);
	// index += 4;
	// dot.adcB = (double)ProtocolUtil.composeSpecialMinus(data.subList(index,
	// index+4).toArray(new Byte[0]), true) / Math.pow(10, KB_FACTOR);
	// index += 4;
	// dot.da = ProtocolUtil.composeSpecialMinus(data.subList(index,
	// index+2).toArray(new Byte[0]), true);
	// index += 2;
	// dot.meter = (double)ProtocolUtil.composeSpecialMinus(data.subList(index,
	// index+3).toArray(new Byte[0]), true) /100;
	// index += 3;
	//
	// dot.programK1 = (double)ProtocolUtil.composeSpecialMinus(data.subList(index,
	// index+4).toArray(new Byte[0]), true) / Math.pow(10, KB_FACTOR);
	// index += 4;
	// dot.programB1 = (double)ProtocolUtil.composeSpecialMinus(data.subList(index,
	// index+4).toArray(new Byte[0]), true) / Math.pow(10, KB_FACTOR);
	// index += 4;
	//
	// dot.programK2 = (double)ProtocolUtil.composeSpecialMinus(data.subList(index,
	// index+2).toArray(new Byte[0]), true) / Math.pow(10, SUB_KB_FACTOR);
	// index += 2;
	// dot.programB2 = (double)ProtocolUtil.composeSpecialMinus(data.subList(index,
	// index+2).toArray(new Byte[0]), true) / Math.pow(10, SUB_KB_FACTOR);
	// index += 2;
	//
	// dot.programK3 = (double)ProtocolUtil.composeSpecialMinus(data.subList(index,
	// index+2).toArray(new Byte[0]), true) / Math.pow(10, SUB_KB_FACTOR);
	// index += 2;
	// dot.programB3 = (double)ProtocolUtil.composeSpecialMinus(data.subList(index,
	// index+2).toArray(new Byte[0]), true) / Math.pow(10, SUB_KB_FACTOR);
	// index += 2;
	//
	// list.add(dot);
	...
	public Code getCode() {
		...
		return LogicCode.HKWriteCalFlashCode;
	}
