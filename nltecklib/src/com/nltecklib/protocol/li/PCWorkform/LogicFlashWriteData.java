...
/**
 * @author wavy_zheng
 * @version 创建时间：2020年10月30日 下午7:20:13 逻辑板flash写入系数，用于调试
 */
...
//						ProtocolUtil.split((long) (dot.adc * Math.pow(10, Math.pow(10, currentResolution))), 3, true)));
//				data.addAll(
...
//		Collections.sort(dots); // 排序
		...
		for (CalDot dot : dots) {
			...
			data.add((byte) dot.pole.ordinal());// 极性
			data.add((byte) dot.level); // level
			data.addAll(Arrays.asList(ProtocolUtil.split((long) (dot.adc * Math.pow(10, Math.pow(10, currentResolution))), 3, true)));
			data.addAll(Arrays
	...
	public void decode(List<Byte> encodeData) {
		...
		for (int n = 0; n < dotCount; n++) {
			...
			dot.adc = (double) ProtocolUtil.compose(data.subList(index, index + 3).toArray(new Byte[0]), true)
					/ Math.pow(10, Math.pow(10, currentResolution));
			index += 3;
	...
	public Code getCode() {
		// TODO Auto-generated method stub
		return PCWorkformCode.LogicFlashWriteCode;
	}
	...
	public void appendDot(CalDot dot) {
