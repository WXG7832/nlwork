...
public class ByteUtils {
    ...
    /**
     * 把byte数组转成list,高位在后的逆排序
     */
    public static List<Integer> byteToBitList(byte b) {
        List<Integer> bits = new ArrayList<>();
        for (int i =  0; i < 8; i++) {	
        	bits.add((int)((b >> i) & 0x1));   
		}
    	return bits;
    }
    /**
    ...
    public static void main(String[] args) {
    	byte b = 110;
    	System.out.println(""+(byte) ((b >> 7) & 0x1)+(byte) ((b >> 6) & 0x1)+(byte) ((b >> 5) & 0x1)+(byte)((b >> 4) & 0x1)
    			+(byte)((b >> 3) & 0x1)+(byte)((b >> 2) & 0x1)+(byte)((b >> 1) & 0x1)+(byte)((b >> 0) & 0x1));
    	System.out.println((byteToBitList(b)));
	}
}
