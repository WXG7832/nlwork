...
public class DriverCalculateData extends Data implements Configable, Queryable, Responsable {
	...
	public void encode() {
        ...
        if(Data.isUseHugeModuleCount()) {
			...
			data.addAll(Arrays.asList(ProtocolUtil.split(convertIntBit(mouduleIndex), 4, true)));
		} else {
		    data.add((byte) convertBit(mouduleIndex));
		}
	...
	public void decode(List<Byte> encodeData) {
        ...
        if(Data.isUseHugeModuleCount()) {
			...
			index += 4;
			mouduleIndex = (byte) convertIndex(mv);
			
		} else {
			
			mouduleIndex = (byte)convertIndex(data.get(index++));
		}
	...
	@Override
	public String toString() {
		return "DriverCalculateData [mouduleIndex=" + mouduleIndex + ", pole=" + pole + ", mode=" + mode
