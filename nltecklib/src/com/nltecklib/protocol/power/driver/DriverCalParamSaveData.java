 ...
 */
public class DriverCalParamSaveData extends Data implements Configable, Queryable, Responsable {
   
	public final static int  K_BIT = 7;
	 ...
	 * 分类存储
	 */
	private Map<CalMode, List<CalParamData>> saveDataMap = new HashMap<>();
	/**
	...
	public void encode() {
		...
		// SLP dot count
		data.add((byte) (saveDataMap.get(CalMode.SLEEP) == null ? 0 : saveDataMap.get(CalMode.SLEEP).size()));
		// cc
		data.add((byte) (saveDataMap.get(CalMode.CC) == null ? 0 : saveDataMap.get(CalMode.CC).size()));
		// cv
		data.add((byte) (saveDataMap.get(CalMode.CV) == null ? 0 : saveDataMap.get(CalMode.CV).size() - cv1DotCount - cv2DotCount));
		// dc
		data.add((byte) (saveDataMap.get(CalMode.DC) == null ? 0 : saveDataMap.get(CalMode.DC).size()));
		// cv1
		...
		// 存入slp各个校准点
		List<CalParamData> list = saveDataMap.get(CalMode.SLEEP) == null ? new ArrayList<>()
				: saveDataMap.get(CalMode.SLEEP);
		encodeSaveDots(list);
		// 存储cc各个校准点
		list = saveDataMap.get(CalMode.CC) == null ? new ArrayList<>() : saveDataMap.get(CalMode.CC);
		encodeSaveDots(list);
		// 存储cv各个校准点
		list = saveDataMap.get(CalMode.CV) == null ? new ArrayList<>() : saveDataMap.get(CalMode.CV);
		encodeSaveDots(list.subList(0, list.size() - cv1DotCount - cv2DotCount));

		cvDotCount = list.size() - cv1DotCount - cv2DotCount;
		// 存储dc各个校准点
		list = saveDataMap.get(CalMode.DC) == null ? new ArrayList<>() : saveDataMap.get(CalMode.DC);
		encodeSaveDots(list);
	...
	private void encodeSaveDots(List<CalParamData> list) {
		...
		for (CalParamData dot : list) {
			...
			data.addAll(Arrays.asList(ProtocolUtil.splitSpecialMinus((long) (dot.adcB * Math.pow(10, B_BIT)), 4, true)));
			...
			data.addAll(Arrays.asList(ProtocolUtil.splitSpecialMinus((long) (dot.programB * Math.pow(10, B_BIT)), 4, true)));
		}
	...
	public int getDotCount() {
		...
		int totalCount = 0;
		for (Iterator<CalMode> it = saveDataMap.keySet().iterator(); it.hasNext();) {

			CalMode mode = it.next();
	...
	public void decode(List<Byte> encodeData) {
		...
		saveDataMap.put(CalMode.SLEEP, list);
		...
		saveDataMap.put(CalMode.CC, list);
		...
		saveDataMap.put(CalMode.CV, list);
		...
		saveDataMap.put(CalMode.DC, list);
		...
		saveDataMap.get(CalMode.CV).addAll(list);
	...
	public Code getCode() {
		// TODO Auto-generated method stub
		return DriverCode.FlashParamCode;
	}
	...
	public Map<CalMode, List<CalParamData>> getSaveDataMap() {
		return saveDataMap;
	}

	public void saveDatas(CalMode mode, List<CalParamData> datas) {
