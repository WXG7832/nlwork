...
public class CommonUtil {
	...
	public static boolean isValidUuid(String uuid) {
		...
		StringBuffer buffZero = new StringBuffer();
		StringBuffer buffEmpty = new StringBuffer();
		for(int n = 0 ; n < 32 ; n++) {
			
			buffZero.append("0");
			buffEmpty.append("f");
		}
		if(uuid.toLowerCase().equals(buffZero.toString())) {
			
			return false;
		}
        if(uuid.toLowerCase().equals(buffEmpty.toString())) {
	 ...
	 * @param executor
	 */
	public static void exitThread(ScheduledExecutorService executor , int waitTime) {
		...
		try {
			...
			if(!executor.awaitTermination(waitTime, TimeUnit.MILLISECONDS)) {
			   ...
			   Environment.infoLogger.info("executor exit!!");
			}
