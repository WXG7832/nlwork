...
import com.nlteck.calModel.base.CalculatePlanDataMoudle;
import com.nlteck.calModel.base.DelayConfig;
 ...
 * 
 * @author wang_xingguo
 * @Description:从3代校准主控复制，去除core 加载chnMap
 * @date 2022年4月11日
 *
 */
public class DiskService {
	...
	private final static String CAL_CONFIG_PATH = "calCfg\\workformCfg";
	...
	private final static String CALCULATEMOUDLE_PLAN_PATH=CAL_CONFIG_PATH+"/calculatePlanMoudle.xml";
...
//		});
	...
	public CalibratePlanData loadCalibratePlan() {
		...
		try {
			...
			File file = new File(CALIBRATE_PLAN_PATH);
...
//			Document doc = XmlUtil.loadXml(CALIBRATE_PLAN_PATH);
	 ...
	 * @return
	 */
	public CalculatePlanDataMoudle loadCalculatePlanMoudle() {
		try {
			CalculatePlanDataMoudle data = new CalculatePlanDataMoudle();
			Document doc = XmlUtil.loadXml(CALCULATEMOUDLE_PLAN_PATH);
			Element rootElement = doc.getRootElement();
			...
			List<Element> modeElements = rootElement.elements("mode");
			modeElements.stream().forEach(me -> {
				CalculatePlanMoudle mode = new CalculatePlanMoudle();
				data.getModes().add(mode);
				mode.mode = CalMode.valueOf(me.attributeValue("mode"));
				mode.pole = Pole.valueOf(me.attributeValue("pole"));
				mode.disabled = Boolean.parseBoolean(me.attributeValue("disabled"));
				mode.moudleIndex=Integer.parseInt(me.attributeValue("module"));
				List<Element> dotElements = me.elements("dot");
				dotElements.stream().forEach(de -> {
					mode.dots.add(Double.parseDouble(de.getText()));
				});
			...
			logger.error("loadCalculatePlanMoudle error:" + e.getMessage(), e);
		}
